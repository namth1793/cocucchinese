...
router.post('/:id/audio', requireAuth, requireRole('admin', 'teacher'), mediaUpload.single('file'), asyncHandler(async (req, res) => {
  const dialogue = await db.find('dialogues', req.params.id);
  ...
  try {
    ...
    const updated = await db.update('dialogues', dialogue.id, { audioUrl: url });
    res.json(updated);
  } catch (e) {
    console.error(e);
...
router.delete('/:id/audio', requireAuth, requireRole('admin', 'teacher'), asyncHandler(async (req, res) => {
  const dialogue = await db.find('dialogues', req.params.id);
  if (!dialogue) return res.status(404).json({ error: 'Không tìm thấy bài khoá' });
  const updated = await db.update('dialogues', dialogue.id, { audioUrl: null });
  res.json(updated);
}));
