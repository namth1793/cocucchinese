...
router.post('/:id/file', requireAuth, requireRole('admin', 'teacher'), sourceFileUpload.single('file'), async (req, res) => {
  const item = db.find('examPapers', req.params.id);
  ...
  try {
    ...
    const updated = db.update('examPapers', item.id, { url });
    res.json(updated);
  } catch (e) {
    console.error(e);
    fs.unlink(req.file.path, () => {});
