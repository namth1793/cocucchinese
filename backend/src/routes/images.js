...
router.get('/', requireAuth, asyncHandler(async (req, res) => {
  let items = await db.all('images');
  if (req.query.category) items = items.filter((i) => i.category === req.query.category);
  res.json(items);
}));

router.post('/', requireAuth, requireRole('admin', 'teacher'), mediaUpload.single('file'), asyncHandler(async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Thiếu file' });
  try {
    const { url } = await storage.saveMedia(req.file.buffer, req.file.originalname, req.file.mimetype);
    const item = await db.insert('images', {
      url,
      category: req.body.category || 'khac',
      caption: req.body.caption || '',
      uploadedBy: req.user.id
    });
    await db.logActivity(req.user.id, 'upload_image', { imageId: item.id });
...
router.delete('/:id', requireAuth, requireRole('admin', 'teacher'), asyncHandler(async (req, res) => {
  const ok = await db.remove('images', req.params.id);
