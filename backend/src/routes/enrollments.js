...
function view(e) {
  const user = db.find('users', e.userId);
  ...
  return {
    ...
    progressPercent: level ? access.courseProgressPercent(e.userId, e.levelId) : 0
  };
}

router.get('/', (req, res) => {
  let items = db.all('enrollments');
  ...
  res.json([...items].reverse().map(view));
});
...
router.post('/', (req, res) => {
  ...
  if (!db.find('levels', levelId)) return res.status(404).json({ error: 'Không tìm thấy khoá học' });
  const existing = svc.findUserByEmail(email);
  if (existing && existing.role !== 'student') return res.status(400).json({ error: 'Email này thuộc tài khoản giáo viên/quản trị' });
  const { user, created, tempPassword } = svc.ensureStudent({ name, email });
  const enrollment = svc.grantEnrollment({ user, levelId, grantedBy: req.user.id, note: note || 'Cấp thủ công' });
  db.logActivity(req.user.id, 'enrollment_grant', { email, levelId });
  res.status(201).json({ enrollment: view(enrollment), accountCreated: created, tempPassword });
});
...
router.put('/:id/status', (req, res) => {
  ...
  const enr = db.find('enrollments', req.params.id);
  ...
  if (status === ENROLLMENT_STATUS.REVOKED) {
    ...
    const stillActive = db.findWhere('enrollments', (e) => e.userId === enr.userId && e.id !== enr.id && access.ACTIVE_STATUSES.includes(e.status));
    if (stillActive.length === 0) db.update('users', enr.userId, { activeSessions: [] });
  } else {
    patch.revokedAt = null;
  }
  const updated = db.update('enrollments', enr.id, patch);
  db.logActivity(req.user.id, 'enrollment_status', { enrollmentId: enr.id, status });
