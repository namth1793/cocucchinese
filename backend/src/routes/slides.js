...
router.post('/', requireAuth, requireRole('admin', 'teacher'), asyncHandler(async (req, res) => {
  ...
  res.status(201).json(await db.insert('slides', { lessonId, title, pages: [], version: 1 }));
}));

router.post('/:id/pages', requireAuth, requireRole('admin', 'teacher'), slideUpload.array('pages', 80), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const updated = await db.update('slides', slide.id, { pages, version: (slide.version || 1) + 1 });
    await db.logActivity(req.user.id, 'upload_slide_pages', { slideId: slide.id, count: newPages.length });
    res.json(updated);
  } catch (e) {
    console.error(e);
...
router.post('/:id/source', requireAuth, requireRole('admin', 'teacher'), sourceFileUpload.single('file'), asyncHandler(async (req, res) => {
  const slide = await db.find('slides', req.params.id);
  if (!slide) return res.status(404).json({ error: 'Không tìm thấy bài giảng' });
  if (!req.file) return res.status(400).json({ error: 'Thiếu file' });
  try {
    await storage.saveSlideSource(slide.id, req.file.path, req.file.originalname, req.file.mimetype);
    const updated = await db.update('slides', slide.id, { sourceOriginalName: req.file.originalname });
    await db.logActivity(req.user.id, 'upload_slide_source', { slideId: slide.id, name: req.file.originalname });
...
async function runConvertPptxInBackground(slideId, file, userId) {
  try {
    ...
    const current = await db.find('slides', slideId);
    const newPages = uploaded.map(({ key }, i) => ({ pageNum: current.pages.length + i + 1, fileName: key }));
    const pages = [...current.pages, ...newPages];
    await db.update('slides', slideId, { pages, version: (current.version || 1) + 1 });

    await storage.saveSlideSource(slideId, file.path, file.originalname, file.mimetype);
    await db.update('slides', slideId, { sourceOriginalName: file.originalname, convertStatus: { state: 'done' } });

    await db.logActivity(userId, 'convert_pptx', { slideId, pages: newPages.length, name: file.originalname });
  } catch (e) {
    console.error('convert-pptx failed:', e?.response?.data || e.message || e);
    await db.update('slides', slideId, {
...
router.post('/:id/convert-pptx', requireAuth, requireRole('admin', 'teacher'), sourceFileUpload.single('file'), asyncHandler(async (req, res) => {
  const slide = await db.find('slides', req.params.id);
  if (!slide) return res.status(404).json({ error: 'Không tìm thấy bài giảng' });
  if (!req.file) return res.status(400).json({ error: 'Thiếu file' });
  ...
  await db.update('slides', slide.id, { convertStatus: { state: 'processing' } });
...
router.get('/:id/convert-status', requireAuth, requireRole('admin', 'teacher'), asyncHandler(async (req, res) => {
  const slide = await db.find('slides', req.params.id);
  if (!slide) return res.status(404).json({ error: 'Không tìm thấy bài giảng' });
...
router.get('/:id/source', requireAuth, requireRole('admin', 'teacher'), asyncHandler(async (req, res) => {
  const slide = await db.find('slides', req.params.id);
...
router.get('/:id/page/:n', asyncHandler(async (req, res) => {
  ...
  if (slide && !access.isStaff(user) && !(await access.canAccessLesson(user, slide.lessonId))) {
...
router.delete('/:id', requireAuth, requireRole('admin', 'teacher'), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await db.remove('slides', slide.id);
    await db.logActivity(req.user.id, 'delete_slide_deck', { slideId: slide.id, title: slide.title });
    res.json({ ok: true });
  } catch (e) {
    console.error(e);
