...
function runMigrations() {
  ...
  if (db.findWhere('migrations', (m) => m.key === key)[0]) return;

  const levels = db.all('levels');
  const students = db.findWhere('users', (u) => u.role === 'student');
  let granted = 0;
  students.forEach((u) => {
    levels.forEach((lv) => {
      const has = db.findWhere('enrollments', (e) => e.userId === u.id && e.levelId === lv.id)[0];
      if (has) return;
  ...
  levels.forEach((lv) => {
    ...
    const lessonCount = db.findWhere('lessons', (l) => l.levelId === lv.id).length;
    if (lv.price === undefined && SAMPLE_PRICES[code] && lessonCount >= MIN_LESSONS_TO_SELL) {
      db.update('levels', lv.id, { price: SAMPLE_PRICES[code] });
    }
  });

  db.insert('migrations', { key, ranAt: new Date().toISOString() });
...
async function applyDefaultCovers() {
  ...
  for (const lv of db.all('levels')) {
    ...
    db.update('levels', lv.id, { coverUrl: url });
    applied += 1;
  }
  db.insert('migrations', { key, ranAt: new Date().toISOString() });
